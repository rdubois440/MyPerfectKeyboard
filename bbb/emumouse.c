...
//	"mkdir /sys/kernel/config/usb_gadget/mouse",
//	"echo 0x1234 >/sys/kernel/config/usb_gadget/mouse/idVendor",
//	"echo 0x5678 >/sys/kernel/config/usb_gadget/mouse/idProduct",
//	"echo 0x0100 >/sys/kernel/config/usb_gadget/mouse/bcdDevice",
//	"echo 0x0110 >/sys/kernel/config/usb_gadget/mouse/bcdUSB",
//	"mkdir /sys/kernel/config/usb_gadget/mouse/configs/c.1",
//	// TODO strings
//	"echo 500 >/sys/kernel/config/usb_gadget/mouse/configs/c.1/MaxPower",
//
//	"mkdir /sys/kernel/config/usb_gadget/mouse/functions/hid.usb0",
//	"echo 1 >/sys/kernel/config/usb_gadget/mouse/functions/hid.usb0/subclass", // boot interface subclass
//	"echo 1 >/sys/kernel/config/usb_gadget/mouse/functions/hid.usb0/protocol", // keyboard
//	"echo 8 >/sys/kernel/config/usb_gadget/mouse/functions/hid.usb0/report_length",
//
//	NULL, // marker to set the record descriptor
//
//	"ln -s /sys/kernel/config/usb_gadget/mouse/functions/hid.usb0 /sys/kernel/config/usb_gadget/mouse/configs/c.1",
//
//	"echo musb-hdrc.1.auto >/sys/kernel/config/usb_gadget/mouse/UDC",
//};

static const char *cmds[] = {

	"mkdir /sys/kernel/config/usb_gadget/kb/functions/hid.usb1",
	"echo 1 >/sys/kernel/config/usb_gadget/kb/functions/hid.usb1/subclass", // boot interface subclass
	"echo 2 >/sys/kernel/config/usb_gadget/kb/functions/hid.usb1/protocol", // mouse
	"echo 8 >/sys/kernel/config/usb_gadget/kb/functions/hid.usb1/report_length",

	NULL, // marker to set the record descriptor

	"ln -s /sys/kernel/config/usb_gadget/kb/functions/hid.usb1 /sys/kernel/config/usb_gadget/kb/configs/c.1",

	//"echo musb-hdrc.0.auto >/sys/kernel/config/usb_gadget/kb/UDC",
};

static const char *remove_cmds[] = {
	"rm /sys/kernel/config/usb_gadget/kb/configs/c.1/hid.usb1",
	"rmdir /sys/kernel/config/usb_gadget/kb/functions/hid.usb1",
};
	...
	for (i = 0; i < array_len(cmds); i++) {
		if (cmds[i] == NULL) {
			if (!write_string_to_file(
				"/sys/kernel/config/usb_gadget/kb/functions/hid.usb1/report_desc",
